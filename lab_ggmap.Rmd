...
The `ggmap()` related tutorials are just shown how to do it without the actual plots! You need to register an API with google to be able to do this! and follow the instructions below: 
...
<i class="fas fa-exclamation-circle"></i> Note &nbsp; To use Google maps, it is necessary to get an API key from [google](https://developers.google.com/maps/gmp-get-started) for your account and then register the API key in your R using the function `register_google()`
...
ggmap(runmap1) + 
  geom_path(data = run_df, aes(color = elevation), size = 3, lineend = "round") + 
